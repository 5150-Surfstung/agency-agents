#!/usr/bin/env sh
# Enforces the rules in capture.md. Run before every commit.
cd "$(dirname "$0")" || exit 1
fail=0
for req in doctrine.md catalog.md capture.md SKILL.md; do
  [ -f "$req" ] || { echo "missing: $req"; fail=1; }
done
...
for s in $(grep -o 'shelves/[a-z0-9-]*\.md' catalog.md capture.md | cut -d: -f2 | sort -u); do
