...
- Deck (20 slides): title → **host intro (count-up stats: 1,800 inspections since 2004, 18 yrs selling, 346 multifamily units, Surfstung 2 yrs)** → **comfort poll (icebreaker)** → **using-AI poll** → **open floor (brag or confess)** → time poll → split → where-AI-pays → farming demo (real index numbers) → **pricing showdown (room vs machine vs record; podium 100/50/25)** → poll → assistant demo → **Stump (house counter + jersey attribution)** → T2K demo → **the Seed 2.0** → **THE BOARD (standings + ring)** → playbook → the room → ladder (lead capture + one-tap email to mike@mikeolsonre.com cc melaniejager@thejagergroup.com) → close (final board top-3 on phones, eXp attribution).
- **The meta-game**: every phone suits up (initials + emoji). Points DERIVE from real artifacts (10/vote · 15/stump question, first 3 · best ring ×10 · podium awards, presenter-only + idempotent) — no client-callable "add points" exists. Polls show LIVE percentages while open, on projector and voted phones, with anonymity stated on screen. Phones mirror every slide; price reveal and board rank render as screenshot cards.
